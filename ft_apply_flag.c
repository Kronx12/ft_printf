...
/*   Updated: 2019/12/11 05:10:39 by gbaud       ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
{
	if (((*s)->len - (ft_strchr((*s)->str, '-') ? 1 : 0)) < lst->precision
		&& ft_strchr("diuxX", lst->type))
	{
		(*s) = ft_structfjoin(get_zero(lst->precision - (*s)->len +
			(ft_strchr((*s)->str, '-') ? 1 : 0)), (*s), 2);
		if (ft_strchr((*s)->str, '-'))
			ft_replace((*s)->str, '-', '0');
	}
	else if ((*s)->len > lst->precision &&
		lst->precision > 0 && ft_strchr("s", lst->type))
		(*s) = ft_strndup((*s), lst->precision, 1);
	else if (lst->precision == 0 && ft_strchr("cspdiuxX", lst->type))
	{
		if (((*s)->str[0] == '0' || lst->type == 's') &&
			ft_strchr("csdiuxX", lst->type))
